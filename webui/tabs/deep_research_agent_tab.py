...
async def run_deep_research(
    ...
    if os.path.commonpath(
        ...
        logger.warning(
            f"Unsafe base_save_dir detected: {base_save_dir}. Using default directory."
        )
        normalized_base_save_dir = os.path.abspath(safe_root_dir)
...
async def stop_deep_research(webui_manager: WebuiManager) -> dict[Component, Any]:
    ...
    if agent and task and not task.done():
        ...
        if report_file_path and os.path.exists(report_file_path):
            ...
            final_update[markdown_display_comp] = gr.update(
                value="# Research Stopped by User"
            )
        ...
        final_update = {
            ...
            webui_manager.get_component_by_id(
                "deep_research_agent.max_iteration"
            ): gr.update(interactive=True),
            webui_manager.get_component_by_id(
                "deep_research_agent.max_query"
            ): gr.update(interactive=True),
...
def create_deep_research_agent_tab(webui_manager: WebuiManager):
    ...
    with gr.Group():
        ...
        with gr.Row():
            resume_task_id = gr.Textbox(
                label="Resume Task ID", value="", interactive=True
            )
    ...
    # --- Connect Handlers ---
    start_button.click(
        fn=start_wrapper, inputs=all_managed_inputs, outputs=dr_tab_outputs
    )
